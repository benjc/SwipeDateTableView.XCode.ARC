//
//  SwipeDateTableViewCell.m
//  Cinetimes
//
//  Created by Benji on 19/12/2013.
//  Copyright (c) 2013 Benji. All rights reserved.
//
...
-(id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        //Defaults
        _timelinePadding = 60;
        _timelineDatePadding = 0;
        timelineScrollingMultiplier = 1.5;
...
-(void)swipeLeft:(UIPanGestureRecognizer *)sender
{
    ...
    switch ([sender state]) {
        case UIGestureRecognizerStateBegan:
            ...
            if (delegate != nil && [delegate respondsToSelector:@selector(swipeDateTableViewCellBeganDragging:)]) {
                [delegate swipeDateTableViewCellBeganDragging:self];
            }
        ...
        case UIGestureRecognizerStateEnded: {
            if (delegate != nil && [delegate respondsToSelector:@selector(swipeDateTableViewCellEndedDragging:)]) {
                [delegate swipeDateTableViewCellEndedDragging:self];
            }
                                                      ...
                                                      completion:^(BOOL finished2) {
                                                          ...
                                                          if (isInActiveArea) {
                                                              if ([dateView _enabled]) {
                                                                  if (delegate != nil && [delegate respondsToSelector:@selector(swipeDateTableViewCell:didTriggerStep:)]) {
                                                                      [delegate swipeDateTableViewCell:weakSelf didTriggerStep:selectedDateIndex];
                                                                  }
                                                              }
                                                          }
                                                      }
